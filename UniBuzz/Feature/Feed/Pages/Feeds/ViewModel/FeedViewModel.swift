...
class FeedViewModel {
    ...
    func fetchData() {
        ...
        feedsData = [Buzz]()
        ServiceConstant.COLLECTION_USERS.document(uid).getDocument { documentSnapshot, err in
            guard let data = documentSnapshot?.data() else { return }
            guard let upvotedFeeds = data["upvotedFeeds"] as? [String] else { return }
            
            ServiceConstant.COLLECTION_FEEDS.order(by: "timestamp", descending: true).getDocuments { querySnapshot, err in
